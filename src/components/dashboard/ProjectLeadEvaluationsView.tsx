...
import type { Employee, ProjectLeadEvaluation, ProjectLeadEvaluationInput } from '@/lib/types'
...
const DIMENSIONS = [
  { key: 'dim1', label: 'On-Time Delivery', shortLabel: 'On-Time', description: 'Sprint completion ≥85%, milestones on track' },
  { key: 'dim2', label: 'Client Satisfaction', shortLabel: 'Client Sat.', description: 'Feedback ≥8/10, zero unplanned escalations' },
  { key: 'dim3', label: 'Budget & Scope', shortLabel: 'Budget/Scope', description: 'Within ±5% budget, scope changes same-day' },
  { key: 'dim4', label: 'Team Leadership', shortLabel: 'Team Lead', description: 'Weekly 1:1s, real-time feedback, morale' },
  { key: 'dim5', label: 'Expansion Signals', shortLabel: 'Expansion', description: '1+ upsell / growth opportunity identified' },
] as const
...
// ─── Form ───
interface FormState {
  project_lead_id: string; month: string
  dim1_score: Score | null; dim1_evidence: string; dim1_sprint_completion: string; dim1_milestones_at_risk: string
  dim2_score: Score | null; dim2_evidence: string; dim2_client_escalations: string; dim2_escalations_resolved: string
  dim3_score: Score | null; dim3_evidence: string; dim3_budget_status: string; dim3_scope_changes: string; dim3_scope_communicated: string
  dim4_score: Score | null; dim4_evidence: string; dim4_ones_held: string; dim4_ones_planned: string; dim4_morale_concerns: string
  dim5_score: Score | null; dim5_evidence: string; dim5_expansion_opportunities: string; dim5_expansion_details: string
  overall_notes: string
}

function emptyForm(month: string, leadId = ''): FormState {
  return {
    project_lead_id: leadId, month,
    dim1_score: null, dim1_evidence: '', dim1_sprint_completion: '', dim1_milestones_at_risk: '',
    dim2_score: null, dim2_evidence: '', dim2_client_escalations: '', dim2_escalations_resolved: '',
    dim3_score: null, dim3_evidence: '', dim3_budget_status: '', dim3_scope_changes: '', dim3_scope_communicated: '',
    dim4_score: null, dim4_evidence: '', dim4_ones_held: '', dim4_ones_planned: '', dim4_morale_concerns: '',
    dim5_score: null, dim5_evidence: '', dim5_expansion_opportunities: '', dim5_expansion_details: '',
    overall_notes: '',
  }
}

function evalToForm(ev: ProjectLeadEvaluation): FormState {
  return {
    project_lead_id: ev.project_lead_id, month: ev.month.slice(0, 7),
    dim1_score: ev.dim1_score, dim1_evidence: ev.dim1_evidence || '', dim1_sprint_completion: ev.dim1_sprint_completion?.toString() || '', dim1_milestones_at_risk: ev.dim1_milestones_at_risk || '',
    dim2_score: ev.dim2_score, dim2_evidence: ev.dim2_evidence || '', dim2_client_escalations: ev.dim2_client_escalations?.toString() || '', dim2_escalations_resolved: ev.dim2_escalations_resolved || '',
    dim3_score: ev.dim3_score, dim3_evidence: ev.dim3_evidence || '', dim3_budget_status: ev.dim3_budget_status?.toString() || '', dim3_scope_changes: ev.dim3_scope_changes?.toString() || '', dim3_scope_communicated: ev.dim3_scope_communicated || '',
    dim4_score: ev.dim4_score, dim4_evidence: ev.dim4_evidence || '', dim4_ones_held: ev.dim4_ones_held?.toString() || '', dim4_ones_planned: ev.dim4_ones_planned?.toString() || '', dim4_morale_concerns: ev.dim4_morale_concerns || '',
    dim5_score: ev.dim5_score, dim5_evidence: ev.dim5_evidence || '', dim5_expansion_opportunities: ev.dim5_expansion_opportunities?.toString() || '', dim5_expansion_details: ev.dim5_expansion_details || '',
    overall_notes: ev.overall_notes || '',
  }
}

function formToInput(f: FormState): ProjectLeadEvaluationInput {
  return {
    project_lead_id: f.project_lead_id, month: f.month + '-01',
    dim1_score: f.dim1_score, dim1_evidence: f.dim1_evidence || null, dim1_sprint_completion: f.dim1_sprint_completion ? Number(f.dim1_sprint_completion) : null, dim1_milestones_at_risk: f.dim1_milestones_at_risk || null,
    dim2_score: f.dim2_score, dim2_evidence: f.dim2_evidence || null, dim2_client_escalations: f.dim2_client_escalations ? Number(f.dim2_client_escalations) : null, dim2_escalations_resolved: f.dim2_escalations_resolved || null,
    dim3_score: f.dim3_score, dim3_evidence: f.dim3_evidence || null, dim3_budget_status: f.dim3_budget_status ? Number(f.dim3_budget_status) : null, dim3_scope_changes: f.dim3_scope_changes ? Number(f.dim3_scope_changes) : null, dim3_scope_communicated: f.dim3_scope_communicated || null,
    dim4_score: f.dim4_score, dim4_evidence: f.dim4_evidence || null, dim4_ones_held: f.dim4_ones_held ? Number(f.dim4_ones_held) : null, dim4_ones_planned: f.dim4_ones_planned ? Number(f.dim4_ones_planned) : null, dim4_morale_concerns: f.dim4_morale_concerns || null,
    dim5_score: f.dim5_score, dim5_evidence: f.dim5_evidence || null, dim5_expansion_opportunities: f.dim5_expansion_opportunities ? Number(f.dim5_expansion_opportunities) : null, dim5_expansion_details: f.dim5_expansion_details || null,
...
function EvalModal({ open, onClose, onSave, evaluation, leadId, month, leads }: {
  open: boolean; onClose: () => void; onSave: (i: ProjectLeadEvaluationInput) => Promise<void>
  evaluation: ProjectLeadEvaluation | null; leadId: string | null; month: string; leads: Employee[]
}) {
  ...
  const handleSave = async () => {
    if (!form.project_lead_id || !form.month) return
    setSaving(true)
      ...
      content: (
        <div className="space-y-4">
          <div className="bg-emerald-50/70 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-lg px-3 py-2.5">
            ...
            <p className="text-xs text-emerald-700 dark:text-emerald-300">Sprint completion &gt;=85% (rolling 4-sprint avg). Milestones on track.</p>
          </div>
          ...
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Sprint completion rate this month (%)</Label><Input type="number" min="0" max="100" value={form.dim1_sprint_completion} onChange={e => set('dim1_sprint_completion', e.target.value)} className="w-32 text-sm h-9" placeholder="e.g. 92" /></div>
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Any milestones at risk?</Label><Textarea value={form.dim1_milestones_at_risk} onChange={e => set('dim1_milestones_at_risk', e.target.value)} rows={2} className="text-sm resize-none" placeholder="Describe any at-risk milestones..." /></div>
        </div>
      ),
    },
    {
      title: '2. Client Satisfaction',
      content: (
        <div className="space-y-4">
          <div className="bg-emerald-50/70 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-lg px-3 py-2.5">
            <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-600 dark:text-emerald-400 mb-1">What Good Looks Like</p>
            <p className="text-xs text-emerald-700 dark:text-emerald-300">Feedback score &gt;=8/10. Zero unplanned escalations.</p>
          </div>
          ...
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Client escalations this month</Label><Input type="number" min="0" value={form.dim2_client_escalations} onChange={e => set('dim2_client_escalations', e.target.value)} className="w-32 text-sm h-9" /></div>
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">How were they resolved?</Label><Textarea value={form.dim2_escalations_resolved} onChange={e => set('dim2_escalations_resolved', e.target.value)} rows={2} className="text-sm resize-none" placeholder="Describe resolution..." /></div>
        </div>
      ),
    },
    {
      title: '3. Budget & Scope',
      content: (
        <div className="space-y-4">
          <div className="bg-emerald-50/70 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-lg px-3 py-2.5">
            <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-600 dark:text-emerald-400 mb-1">What Good Looks Like</p>
            <p className="text-xs text-emerald-700 dark:text-emerald-300">Within +/-5% of budget. Scope changes communicated same-day with impact analysis.</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label className="text-[11px] text-muted-foreground mb-1 block">Budget status (% over/under)</Label><Input type="number" value={form.dim3_budget_status} onChange={e => set('dim3_budget_status', e.target.value)} className="text-sm h-9" placeholder="e.g. -3 or +7" /></div>
            <div><Label className="text-[11px] text-muted-foreground mb-1 block">Scope changes this month</Label><Input type="number" min="0" value={form.dim3_scope_changes} onChange={e => set('dim3_scope_changes', e.target.value)} className="text-sm h-9" /></div>
          </div>
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">All scope changes communicated with impact analysis?</Label><Textarea value={form.dim3_scope_communicated} onChange={e => set('dim3_scope_communicated', e.target.value)} rows={2} className="text-sm resize-none" placeholder="Details on scope communication..." /></div>
        </div>
      ),
    },
    {
      ...
      content: (
        <div className="space-y-4">
          <div className="bg-emerald-50/70 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-lg px-3 py-2.5">
            <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-600 dark:text-emerald-400 mb-1">What Good Looks Like</p>
            <p className="text-xs text-emerald-700 dark:text-emerald-300">Weekly 1:1s with all team members. Real-time feedback happening. No unaddressed morale issues.</p>
          </div>
          ...
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">1:1s held vs. planned</Label>
            <div className="flex items-center gap-2">
              <Input type="number" min="0" value={form.dim4_ones_held} onChange={e => set('dim4_ones_held', e.target.value)} className="w-24 text-sm h-9" placeholder="Held" />
              <span className="text-muted-foreground text-sm">/</span>
              <Input type="number" min="0" value={form.dim4_ones_planned} onChange={e => set('dim4_ones_planned', e.target.value)} className="w-24 text-sm h-9" placeholder="Planned" />
            </div>
          </div>
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Any morale concerns?</Label><Textarea value={form.dim4_morale_concerns} onChange={e => set('dim4_morale_concerns', e.target.value)} rows={2} className="text-sm resize-none" placeholder="Describe concerns or write 'None'..." /></div>
        </div>
      ),
    },
    {
      title: '5. Expansion Signals',
      content: (
        <div className="space-y-4">
          <div className="bg-emerald-50/70 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800 rounded-lg px-3 py-2.5">
            <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-600 dark:text-emerald-400 mb-1">What Good Looks Like</p>
            <p className="text-xs text-emerald-700 dark:text-emerald-300">1+ upsell or growth opportunity identified and communicated to Sales/VPCS this month.</p>
          </div>
          ...
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Expansion opportunities identified</Label><Input type="number" min="0" value={form.dim5_expansion_opportunities} onChange={e => set('dim5_expansion_opportunities', e.target.value)} className="w-32 text-sm h-9" /></div>
          <div><Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Details and estimated value</Label><Textarea value={form.dim5_expansion_details} onChange={e => set('dim5_expansion_details', e.target.value)} rows={2} className="text-sm resize-none" placeholder="Describe opportunities and value..." /></div>
        </div>
      ),
    },
  ]
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader><DialogTitle>{isNew ? 'Add' : 'Edit'} Project Lead Evaluation</DialogTitle></DialogHeader>
        <div className="flex-1 overflow-y-auto px-1">
          <div className="space-y-5 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Project Lead</Label>
                {isNew ? (
                  <Select value={form.project_lead_id} onValueChange={v => set('project_lead_id', v)}>
                    <SelectTrigger className="h-9 text-sm"><SelectValue placeholder="Select project lead..." /></SelectTrigger>
                  ...
                  <div className="flex items-center gap-2 h-9 px-3 rounded-lg bg-muted/50 border border-border">
                    <EmployeeAvatar name={evaluation?.project_lead?.full_name || ''} photoUrl={evaluation?.project_lead?.photo_url} size="sm" />
                    <span className="text-sm font-medium">{evaluation?.project_lead?.full_name}</span>
                  </div>
                )}
              </div>
              <div>
        ...
        <DialogFooter className="pt-4 border-t border-border">
          ...
          <Button onClick={handleSave} disabled={saving || !form.project_lead_id || !form.month} className="bg-[#ea2775] hover:bg-[#d01e65] text-white">
...
function EvalDetailView({ open, evaluation: ev, lead: pl, onClose, onEdit }: {
  open: boolean; evaluation: ProjectLeadEvaluation | null; lead: Employee | null; onClose: () => void; onEdit: () => void
}) {
  if (!ev) return null
  const monthLabel = new Date(ev.month).toLocaleDateString('en-US', { month: 'long', year: 'numeric', timeZone: 'UTC' })

  const dimDetails = [
    { dim: DIMENSIONS[0], score: ev.dim1_score, goodLooksLike: 'Sprint completion >=85% (rolling 4-sprint avg). Milestones on track.', fields: [
      { label: 'Evidence', value: ev.dim1_evidence },
      { label: 'Sprint completion rate', value: ev.dim1_sprint_completion != null ? `${ev.dim1_sprint_completion}%` : null },
      { label: 'Milestones at risk', value: ev.dim1_milestones_at_risk },
    ]},
    { dim: DIMENSIONS[1], score: ev.dim2_score, goodLooksLike: 'Feedback score >=8/10. Zero unplanned escalations.', fields: [
      { label: 'Evidence', value: ev.dim2_evidence },
      { label: 'Client escalations', value: ev.dim2_client_escalations },
      { label: 'How resolved', value: ev.dim2_escalations_resolved },
    ]},
    { dim: DIMENSIONS[2], score: ev.dim3_score, goodLooksLike: 'Within +/-5% of budget. Scope changes communicated same-day with impact analysis.', fields: [
      { label: 'Evidence', value: ev.dim3_evidence },
      { label: 'Budget status', value: ev.dim3_budget_status != null ? `${ev.dim3_budget_status > 0 ? '+' : ''}${ev.dim3_budget_status}%` : null },
      { label: 'Scope changes', value: ev.dim3_scope_changes },
      { label: 'Scope communication', value: ev.dim3_scope_communicated },
    ]},
    { dim: DIMENSIONS[3], score: ev.dim4_score, goodLooksLike: 'Weekly 1:1s with all team members. Real-time feedback happening. No unaddressed morale issues.', fields: [
      { label: 'Evidence', value: ev.dim4_evidence },
      { label: '1:1s held / planned', value: (ev.dim4_ones_held != null && ev.dim4_ones_planned != null) ? `${ev.dim4_ones_held} / ${ev.dim4_ones_planned}` : null },
      { label: 'Morale concerns', value: ev.dim4_morale_concerns },
    ]},
    { dim: DIMENSIONS[4], score: ev.dim5_score, goodLooksLike: '1+ upsell or growth opportunity identified and communicated to Sales/VPCS this month.', fields: [
      { label: 'Evidence', value: ev.dim5_evidence },
      { label: 'Expansion opportunities', value: ev.dim5_expansion_opportunities },
      { label: 'Details & estimated value', value: ev.dim5_expansion_details },
    ]},
  ]
      ...
      <DialogContent showCloseButton={false} className="!max-w-none w-[92vw] max-h-[92vh] overflow-hidden flex flex-col p-0">
        <div className="flex items-start justify-between px-6 pt-5 pb-4 border-b border-border">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-foreground">{pl?.full_name ?? 'Project Lead'}</h2>
        ...
        <div className="flex items-center flex-wrap gap-x-4 gap-y-2 px-6 py-3 bg-muted/30 border-b border-border">
          {DIMENSIONS.map(d => {
            const score = ev[`${d.key}_score` as keyof ProjectLeadEvaluation] as Score | null
            return (
...
// ─── Main View ───
interface ProjectLeadEvaluationsViewProps {
  evaluations: ProjectLeadEvaluation[]
  loading: boolean
  leads: Employee[]
  onAdd: (input: ProjectLeadEvaluationInput) => Promise<void>
  onUpdate: (id: string, input: ProjectLeadEvaluationInput) => Promise<void>
  onDelete: (id: string) => Promise<void>
}

export function ProjectLeadEvaluationsView({ evaluations, loading, leads, onAdd, onUpdate, onDelete }: ProjectLeadEvaluationsViewProps) {
  const { currentUser } = useDashboard()
  const [filterLeadId, setFilterLeadId] = useState('__all__')
  const [filterMonth, setFilterMonth] = useState('__all__')
  const [modal, setModal] = useState<{ open: boolean; evaluation: ProjectLeadEvaluation | null; leadId: string | null }>({ open: false, evaluation: null, leadId: null })
  const [detailView, setDetailView] = useState<ProjectLeadEvaluation | null>(null)
  ...
  const filtered = useMemo(() => {
    ...
    if (filterLeadId !== '__all__') list = list.filter(e => e.project_lead_id === filterLeadId)
  ...
  const handleSave = async (input: ProjectLeadEvaluationInput) => {
    ...
    <>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Project Leads', value: leads.length, colorClass: 'text-violet-700 dark:text-violet-300', bgClass: 'bg-violet-50 dark:bg-violet-950/30' },
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <Select value={filterLeadId} onValueChange={setFilterLeadId}>
            <SelectTrigger className="h-9 w-[220px] text-sm bg-card"><SelectValue placeholder="All Project Leads" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="__all__">All Project Leads</SelectItem>
        ...
        {leads.length === 0 ? (
          <div className="bg-card rounded-xl border border-border flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-base font-semibold text-foreground mb-1">No Project Leads found</h3>
          </div>
        ) : filtered.length === 0 ? (
              ...
              <table className="w-full">
                <thead className="sticky top-0 bg-card z-20">
                  <tr className="border-b border-border">
                    ...
                    <th className="text-left px-3 py-3 w-[170px]"><span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Project Lead</span></th>
                ...
                <tbody>
                  {filtered.map(ev => {
                    const pl = ev.project_lead || leads.find(l => l.id === ev.project_lead_id)
                    return (
                          ...
                          {pl ? (
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                {pl.practice?.name && <p className="text-[11px] text-muted-foreground truncate">{pl.practice.name}</p>}
                              </div>
                            </div>
                          ) : <span className="text-muted-foreground text-xs">—</span>}
                        </td>
                        {DIMENSIONS.map(d => <td key={d.key} className="px-3 py-2.5 w-[80px]"><ScoreBadge score={ev[`${d.key}_score` as keyof ProjectLeadEvaluation] as Score | null} compact /></td>)}
                        ...
                        <td className="px-2 py-2.5 w-[44px]">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-40">
                              ...
                              <DropdownMenuItem onClick={() => setDeleteConfirm({ open: true, id: ev.id, name: pl?.full_name || 'this evaluation' })} className="text-destructive gap-2 text-xs"><Trash2 className="w-3.5 h-3.5" />Delete</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </td>
                      </tr>
      ...
      <EvalDetailView open={!!detailView} evaluation={detailView} lead={detailView ? (detailView.project_lead || leads.find(l => l.id === detailView.project_lead_id) || null) : null} onClose={() => setDetailView(null)}
        onEdit={() => { const ev = detailView; setDetailView(null); setModal({ open: true, evaluation: ev, leadId: null }) }} />
